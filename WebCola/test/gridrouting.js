...
QUnit.test('metro crossing min', function (assert) {
    ...
    assert.deepEqual(lcs.getSequence().map(function (v) { return v.id; }), [0, 1, 2]);
    var e0reversed = edges[0].slice(0).reverse();
    lcs = new cola.LongestCommonSubsequence(e0reversed, edges[1]);
    assert.deepEqual(lcs.getSequence().map(function (v) { return v.id; }), [2, 1, 0]);
...
//# sourceMappingURL=gridrouting.js.map